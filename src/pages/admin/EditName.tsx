...
export default function EditName() {
  ...
  useEffect(() => {
    if (nameSlug) dispatch(fetchNameByIdForAdmin(nameSlug));
  }, [nameSlug, dispatch]);
  ...
  useEffect(() => {
    ...
    getNamesForInput().then((nameSlugs) => setNameOptions(nameSlugs.filter((slug) => slug.slug !== nameSlug)));
  }, [nameSlug]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await editNameById(nameSlug, formDetail);
      //   handleClear();
      setMessageState({ open: true, success: true, message: "Name updated Successfully" });
    } catch (err) {
      const error = err as Error;
      setMessageState({ open: true, success: false, message: `Error updating name: ${error.message}` });
